...
resolve_python_bin() {
  local script_root
  script_root="${TTS_MCP_ROOT_DIR:-$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)}"
...
print_usage() {
  cat <<'EOF'
Usage:
  scripts/install_kokoro_onnx_macos.sh [--lang en|zh] [--profile v1_0|zh_v1_1] [--model-variant int8|fp16|fp16-gpu|full]
...
main() {
  ...
  if [ "$arch" != "x86_64" ] && [ "$arch" != "amd64" ]; then
    echo "This installer currently supports Intel macOS only (x86_64)." >&2
    exit 1
  fi
  ...
  case "$profile" in
    v1_0)
      ...
      echo
      echo "Kokoro runtime installed for Intel macOS."
    ...
    zh_v1_1)
      ...
      echo
      echo "Kokoro Mandarin runtime installed for Intel macOS."
  ...
  echo "  TTS_MCP_BACKEND=auto"
  echo "  KOKORO_TTS_DEFAULT_LANG_CODE=en-us"
}
