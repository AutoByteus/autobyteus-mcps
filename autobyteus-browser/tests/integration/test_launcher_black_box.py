...
def copy_minimal_bundle(destination: Path) -> Path:
    ...
    scripts.mkdir()
    shutil.copy2(CLI_LAUNCHER, scripts / "autobyteus-browser")
    return destination
...
case "${FAKE_UV_MODE:-ready-success}" in
  ready-success)
    printf '%s\n' 'autobyteus-browser-cli-ready-v1' >"$AUTOBYTEUS_BROWSER_CLI_READY_FILE"
...
def test_launcher_readiness_branches_and_missing_bundle_are_exactly_once(tmp_path: Path) -> None:
    ...
    launcher = bundle / "scripts/autobyteus-browser"
