...
CLI_LAUNCHER = PROJECT_ROOT / "scripts" / "autobyteus-browser"
MCP_LAUNCHER = PROJECT_ROOT / "scripts" / "autobyteus-browser-mcp"
...
def chrome_executable() -> Path:
    configured = os.environ.get("AUTOBYTEUS_BROWSER_TEST_CHROME_BIN")
    candidates = [
        configured,
    ...
    raise FileNotFoundError(
        "No supported Chrome/Chromium executable was found; set AUTOBYTEUS_BROWSER_TEST_CHROME_BIN."
    )
        ...
        environment.update(
            {
                ...
                "AUTOBYTEUS_AGENT_WORKSPACE": str(workspace.resolve()),
            }
        )
