...
from autobyteus_browser.errors import BrowserError, invalid_argument
from autobyteus_browser.json_codec import StrictJsonError, dumps_strict, loads_strict

SCHEMA_VERSION = "1"
READY_ENV = "AUTOBYTEUS_BROWSER_CLI_READY_FILE"
READY_TOKEN = "autobyteus-browser-cli-ready-v1"
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        if "--debug" in actual_argv or os.environ.get("AUTOBYTEUS_BROWSER_DEBUG") == "1":
            logging.exception("Unhandled browser CLI failure")
        else:
            print(f"autobyteus-browser: internal failure: {exc}", file=sys.stderr)
