...
from autobyteus_browser.errors import BrowserError, configuration_error, invalid_argument
...
class ArtifactPolicy:
    ...
    def from_environment(cls) -> "ArtifactPolicy":
        configured = os.environ.get("AUTOBYTEUS_AGENT_WORKSPACE")
        if configured is not None:
            if not configured.strip():
                raise configuration_error("AUTOBYTEUS_AGENT_WORKSPACE must be non-empty when set.")
            path = Path(configured).expanduser()
            if not path.is_absolute():
                raise configuration_error("AUTOBYTEUS_AGENT_WORKSPACE must be an absolute path.")
            return cls(path)
        return cls(Path.cwd())
