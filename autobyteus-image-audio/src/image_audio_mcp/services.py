...
logging.getLogger("autobyteus.llm.utils.media_payload_formatter").setLevel(logging.ERROR)
...
async def find_target_coordinates(
        ...
        try:
            fallback_response = await llm.send_user_message(
                LLMUserMessage(
                    ...
                    image_urls=[str(resolved_marked)],
                )
            )
    ...
    ow, oh = original_size
    mw, mh = marked_size
    x_original = max(0.0, min(float(ow - 1), marker_x * float(ow) / float(mw)))
    y_original = max(0.0, min(float(oh - 1), marker_y * float(oh) / float(mh)))
...
async def edit_image(
    ...
    normalized_mask = _normalize_media_source(mask_image, workspace_root) if mask_image else None
